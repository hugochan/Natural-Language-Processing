...
    """
    ...
    for sent in sentences:
        ...
        for i in range(len(sent)):
            ...
            ft.append('1' if contains_name(sent[i]) else '0') # contains_name
            ft.append('1' if contains_nationality(sent[i]) else '0') # contains_nationality
            ft.append('1' if contains_location(sent[i]) else '0') # contains_location
            ft.append('1' if i == 0 else '0') # is_first_word
            ft.append('1' if sent[i].isupper() else '0') # is_upper_case
            ft.append('1' if sent[i].istitle() else '0') # is_title_case
            ft.append('1' if i > 0 and (sent[i-1].isupper() or sent[i-1].istitle()) else '0') # is_pre_upper_or_title
            ft.append('1' if '-' in sent[i] else '0') # has_hyphen
            features.append(ft)
            ctr += 1
    assert ctr == len(text)
    return features
